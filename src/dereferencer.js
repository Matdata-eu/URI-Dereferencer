...
class URIDereferencer {
    ...
    extractURIFromPath() {
        ...
        return window.CONFIG.BASE_URI + '/' + uri;
    }
    ...
    async showRDFFormat(accept, label) {
        ...
        try {
            ...
            const url = window.CONFIG.SPARQL_ENDPOINT + '?query=' + encodeURIComponent(query);
     ...
     */
    makeLocalLink(uri) {
        if (!uri.startsWith(window.CONFIG.ENTITY_NS)) return null;
        // Production: origin matches entity NS → use just the path
        if (window.location.origin === window.CONFIG.ENTITY_NS) {
            return uri.substring(window.CONFIG.ENTITY_NS.length) || '/';
        }
    ...
    async loadResource() {
        try {
            ...
            if (typeof window.initializeGraph === 'function') {
                try {
                    await window.initializeGraph(this.triples, window.CONFIG.SPARQL_ENDPOINT, PREFIXES);
                } catch (graphError) {
    ...
    async checkForGeometry() {
        ...
        try {
            ...
            const url = window.CONFIG.SPARQL_ENDPOINT + '?query=' + encodeURIComponent(wktQuery);
            const response = await fetch(url, {
                headers: {
            ...
            if (data.results.bindings.length > 0) {
                ...
                if (typeof window.initializeMap === 'function') {
                    window.initializeMap(this.wktLiteral);
                }
            }
        } catch (error) {
    ...
    async loadSameClassResources() {
        ...
        try {
            ...
            const url = window.CONFIG.SPARQL_ENDPOINT + '?query=' + encodeURIComponent(sameClassQuery);
            const response = await fetch(url, {
                headers: {
                    'Accept': 'application/sparql-results+json'
                }
